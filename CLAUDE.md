...
- `index.html` — catalogo corsi pubblico (filtri per brand/ruolo, costruito da `COURSES`/`RUOLI`)
...
- `admin.html` — pannello admin: team, corsi assegnati, scadenze, matrice brand, cruscotto competenze, impostazioni
...
- `courses.js` nella root: duplicato obsoleto (171 corsi, con `desc`/`prezzo`). Il catalogo vivo è **`js/courses.js`** (179 corsi).
...
- `corsi_assegnati` — `tech_id`, `brand`, `corso`, `anno`, `stato` (`da_fare`/`in_corso`/`completato`), `scadenza`, `note`, `assegnato_da`, `assegnato_il`
...
Generato da un Excel, poi mantenuto a mano. Espone:
- `COURSES` — `{id, brand, gruppo, nome, erogazione, durata, paths[]}`
- `RUOLI` — 17 percorsi con `{key, label, gruppo}`, chiavi con prefisso `core_*`, `adv_*`, `spec_*`
- `getCorsiPerRuoli(keys)`, `parseRuoli(str)`, `labelRuolo(key)`

`tecnici.ruolo` non è testo libero: contiene le **chiavi dei path separate da virgola** (es. `core_net,adv_eng_net`). Quando l'admin assegna i ruoli a un tecnico, `getCorsiPerRuoli` genera automaticamente le righe in `corsi_assegnati`. Aggiungere un corso significa aggiungere un oggetto a `COURSES` con gli stessi campi e path validi.
