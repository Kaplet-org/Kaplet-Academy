...
`corsi_ai_setup.sql` aggiunge quattro corsi AI gratuiti (Anthropic e OpenAI) sul percorso **`core_ai`** ("Core Path - AI") e mette quel percorso addosso a ogni tecnico attivo: sono obbligatori per chiunque. **Ai nuovi assunti vanno spuntati `core_ai` e `core_compliance` insieme agli altri percorsi**, altrimenti non ricevono la formazione obbligatoria. È una migrazione additiva, `catalogo_setup.sql` resta la fotografia dell'Excel di partenza.

`corsi_compliance_setup.sql` fa lo stesso per ISO 27001 e ISO 9001 (piattaforma Complaion) sul percorso **`core_compliance`**. Due percorsi separati e non un unico "obbligatori" perché sono materie diverse e domani ci si aggiunge GDPR o sicurezza sul lavoro senza mescolarli con l'AI.

Gli script SQL in root si eseguono nel SQL Editor del dashboard (la CLI Supabase non è installata e gli strumenti MCP non hanno permessi di scrittura su questo progetto): `catalogo_setup.sql`, `corsi_ai_setup.sql`, `corsi_compliance_setup.sql`, `mansione_setup.sql`, `registrazione_setup.sql`, `pianificazione_setup.sql`, `audit_log_setup.sql`, `blocca_cancellazioni_setup.sql`, `cron_setup.sql`. Sono idempotenti.
...
- `RUOLI` — 19 percorsi con `{key, label, gruppo}`, chiavi con prefisso `core_*`, `adv_*`, `spec_*`
