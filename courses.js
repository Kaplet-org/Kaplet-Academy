...
// 171 corsi - ogni corso ha i path/ruoli a cui appartiene
// ============================================================
...
function parseRuoli(ruoloStr) {
  ...
  ruoloStr.split(',').map(s => s.trim()).filter(Boolean).forEach(part => {
    if (validKeys.includes(part)) { out.push(part); }
    else { const legacy = LEGACY[part.toLowerCase()]; if (legacy) legacy.forEach(k => out.push(k)); }
  });
  return [...new Set(out)];
}
