...
const COURSES = [
  ...
  },
  {
    ...
    "brand": "",
    "gruppo": "",
    "nome": "controllo delle competenze",
    "erogazione": "",
    "durata": "00:30",
    "paths": []
  },
...
// Compat: accetta una stringa con path separati da virgola (es. "core_net,adv_eng_net")
// Traduce anche i vecchi nomi testuali (es. "Engineer AV | Junior") in path keys.
function parseRuoli(ruoloStr) {
  if (!ruoloStr) return [];
  // Mappa vecchi nomi testuali -> path keys (retrocompatibilità)
  const LEGACY = {
  ...
  const validKeys = RUOLI.map(r => r.key);
  const out = [];
  ruoloStr.split(',').map(s => s.trim()).filter(Boolean).forEach(part => {
    if (validKeys.includes(part)) {
      // è già una path key valida
      out.push(part);
    } else {
      // prova a tradurre da nome testuale vecchio
      const legacy = LEGACY[part.toLowerCase()];
      if (legacy) legacy.forEach(k => out.push(k));
    }
  });
  return [...new Set(out)];
}
